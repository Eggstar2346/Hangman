...
public class Hangman extends Thread
{
	...
	try
	{
	    for (int x = 0 ; x <= 2 ; x++)
	    {
		input = new BufferedReader (new FileReader (filenames [x]));
		if (input.readLine ().equals ("Hangman Program Header"))
		    counter++;
	    }
	}
	catch (IOException e)
	...
	if ((rand >= 1) && (rand <= 10))
	{
	    loadImage ("INTRO BIO.jpg");
	}
	else if ((rand >= 11) && (rand <= 20))
	{
	    loadImage ("INTRO CHEM.jpg");
	}
	else if ((rand >= 21) && (rand <= 30))
	{
	    loadImage ("INTRO PHYSICS.jpg");
	}
	else if ((rand >= 31) && (rand <= 40))
	{
	    loadImage ("INTRO COMP SCI.jpg");
	}
	else if ((rand >= 41) && (rand <= 50))
	{
	    loadImage ("INTRO MATH.jpg");
	}
	else if ((rand >= 51) && (rand <= 60))
	{
	    loadImage ("INTRO ENGLISH.jpg");
	}
	else if ((rand >= 61) && (rand <= 70))
	{
	    loadImage ("INTRO MUSIC.jpg");
	}
	else if ((rand >= 71) && (rand <= 80))
	{
	    loadImage ("INTRO ART.jpg");
	}
	else if ((rand >= 81) && (rand <= 90))
	{
	    loadImage ("INTRO HISTORY.jpg");
	}
	else if ((rand >= 91) && (rand <= 100))
	{
	    loadImage ("INTRO GEO.jpg");
	}
    }
	...
	{
	    loadImage ("INTRO MYSTERY.jpg");
	}
	delay (2000);
	setUp ();
	if (!word.equals ("corrupted"))
	{
	    c.setColor (white);
	...
	{
	    new Message ("Sorry, it looks like someone tampered with our files. We will redirect you to the main menu.", "CORRUPTION ERROR");
	}
    }
	...
	if (yesWin == true)
	{
	    loadImage ("winScreen.jpg");
	}


	else
	{
	    loadImage ("loseScreen.jpg");
	    c.setFont (new Font ("Times New Roman", Font.TRUETYPE_FONT, 25));
	    c.setColor (white);
	...
	if (header.equals ("Hangman Program Header"))
	{
	    while (true)
	    {
		movement = c.getChar ();
    ...
    {
	loadImage ("highscores background.jpg");
	c.setFont (new Font ("Times New Roman", Font.TRUETYPE_FONT, 20));
	c.setColor (black);
	for (int x = 0 ; x <= 9 ; x++)
	{
	...
	if (!maxword)
	{
	    loadImage ("goodbye.jpg");
	    pauseProgram ();
	}
	else
	{
	    loadImage ("WordLimit.jpg");
	    delay (5000);
	}
	c.close ();
    }
	...
	{
	    output = new PrintWriter (new FileWriter ("HighscoresFINAL.txt"));
	    output.println ("Hangman Program Header");
	    for (int x = 0 ; x <= 9 ; x++)
	    {
